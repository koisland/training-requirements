...
# Two groups:
# * HeLa Ctrl and HeLa Salmonella infected MOI 100
...
raw_data <- file.path("data", "rse_gene.Rdata")

load(raw_data)

# ---- Question ----
# Salmonella infection least efficient in G1-arrested cells.
# Cyclin D1 promotes transition from G1 to S.
# miR-15 family of miRNAs decrease cyclin D1 activity.
# miR-15 miRNAs are downregulated on Salmonella infection.
# What genes are enriched by Salmonella infection?
# What miR-15 miRNA are downregulated?

# ---- Workflow ----
rse_gene$condition <- c(
  rep("HeLaCtrl", 3),
  rep("HeLaInfected", 3)
)
cnts <- assay(rse_gene)
...
# ---- Volcano Plot ----
plot_volcano <- EnhancedVolcano(resdf,
  lab = resdf$SYMBOL,
...
# Not the best as uses padj and logfoldchange to determine expression levels
# p cutoff is arbitrary
# can be inflated based on total number of genes over/under expressed
